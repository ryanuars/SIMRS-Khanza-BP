...
public final class DlgRawatJalan extends javax.swing.JDialog {
    ...
    public DlgRawatJalan(java.awt.Frame parent, boolean modal) {
        ...
        tbPemeriksaan.setDefaultRenderer(Object.class, new WarnaTable());
                ...
                // Buat popup menu untuk tabel pemeriksaan
JPopupMenu popupMenuSoap = new JPopupMenu();
popupMenuSoap.setName("popupMenuSoap");
popupMenuSoap.setBackground(new java.awt.Color(255, 255, 254)); // Match background with jPopupMenu1

JMenuItem menuEvaluasiPRB = new JMenuItem();
menuEvaluasiPRB.setBackground(new java.awt.Color(255, 255, 254)); // Same background as MnCetakSuratHamil
menuEvaluasiPRB.setFont(new java.awt.Font("Tahoma", 0, 11)); // Same font as MnCetakSuratHamil
menuEvaluasiPRB.setForeground(new java.awt.Color(50, 50, 50)); // Same foreground as MnCetakSuratHamil
menuEvaluasiPRB.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // Same icon
menuEvaluasiPRB.setText("Evaluasi PRB"); // Set text
menuEvaluasiPRB.setName("menuEvaluasiPRB");
menuEvaluasiPRB.setPreferredSize(new java.awt.Dimension(170, 18)); // Same preferred size as MnCetakSuratHamil

// PERBAIKAN FINAL - Action listener untuk menu Evaluasi PRB
menuEvaluasiPRB.addActionListener(new java.awt.event.ActionListener() {
    public void actionPerformed(java.awt.event.ActionEvent evt) {
        if (tbPemeriksaan.getSelectedRow() != -1) {
            DlgEvaluasiPRB form = new DlgEvaluasiPRB(null, false);
            form.setSize(internalFrame1.getWidth() - 20, internalFrame1.getHeight() - 20);
            form.setLocationRelativeTo(internalFrame1);
            
            // Kirim data dari row yang dipilih
            int row = tbPemeriksaan.getSelectedRow();
            form.setNoRm(
                tbPemeriksaan.getValueAt(row, 1).toString(),  // NoRawat
                tbPemeriksaan.getValueAt(row, 2).toString(),  // NoRM  
                tbPemeriksaan.getValueAt(row, 3).toString(),  // NamaPasien
                tbPemeriksaan.getValueAt(row, 7).toString()   // Tensi
            );
        ...
        // Tambahkan menu ke popup
        popupMenuSoap.add(menuEvaluasiPRB);

        // Set popup menu ke tabel
        tbPemeriksaan.setComponentPopupMenu(popupMenuSoap);

        // Mouse listener untuk menampilkan popup
        tbPemeriksaan.addMouseListener(new java.awt.event.MouseAdapter() {
            @Override
            public void mousePressed(java.awt.event.MouseEvent evt) {
                if (evt.isPopupTrigger()) {
                    int row = tbPemeriksaan.rowAtPoint(evt.getPoint());
                    if (row >= 0 && row < tbPemeriksaan.getRowCount()) {
                        tbPemeriksaan.setRowSelectionInterval(row, row);
                        popupMenuSoap.show(evt.getComponent(), evt.getX(), evt.getY());
                    }
                }
            }
        ...
        // KODE CONTEXT MENU BERAKHIR DI SINI
        // Lanjut ke kode yang sudah ada sebelumnya...
