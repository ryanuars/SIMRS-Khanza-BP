...
private void ChkInputActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChkInputActionPerformed
  isForm();                
    ...
    try {
        ...
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(this, "Pilih data surat kontrol terlebih dahulu!");
            return;
        }
        
        // Ambil data dari table yang dipilih
        String noRawat = tbObat.getValueAt(selectedRow, 0).toString();    // No.Rawat
        String noSEP = tbObat.getValueAt(selectedRow, 1).toString();      // No.SEP  
        String noKartu = tbObat.getValueAt(selectedRow, 2).toString();    // No.Kartu
        String noRM = tbObat.getValueAt(selectedRow, 3).toString();       // No.RM
        String namaPasien = tbObat.getValueAt(selectedRow, 4).toString(); // Nama Pasien
        String noSurat = tbObat.getValueAt(selectedRow, 9).toString();    // No.Surat
        
String noTelp = "";
        
        // Ambil nomor telepon dari tabel pasien
        try {
            ...
            if (rs.next()) {
                ...
                if (rawNoTelp != null && !rawNoTelp.trim().isEmpty()) {
                    // OTOMATIS CONVERT KE FORMAT 62 SAAT AMBIL DARI DATABASE
        ...
        // Buat screenshot
        String fileName = buatScreenshotSuratKontrol(noRM, namaPasien, noSurat);
        if (fileName == null) {
            return;
        }
        ...
        String namaSensor = sensorNamaPasien(namaPasien, 1, 2); // 1-2 karakter awal
        String pesanWA = "Berikut adalah pesan WhatsApp resmi dari RS Umum Banjar Patroman, kami sampaikan untuk Surat Kontrol:\n\n" +
                        "Nama: " + namaSensor;
        ...
        // Tambahkan pesan Mobile JKN
        pesanWA += "\n\nSilahkan melakukan pendaftaran melalui aplikasi Mobile JKN."+
...
private String buatScreenshotSuratKontrol(String noRM, String namaPasien, String noSurat) {
    try {
        // 1. Cari file jasper report
        String jasperPath = cariFileJasper();
        if (jasperPath == null) {
            JOptionPane.showMessageDialog(this, "❌ File rptBridgingSuratKontrol2.jasper tidak ditemukan!");
            return null;
        }
        
        // 2. Parameter report sama seperti sebelumnya
        Map<String, Object> param = new HashMap<>();
        param.put("namars", akses.getnamars());
        param.put("alamatrs", akses.getalamatrs());
        param.put("kotars", akses.getkabupatenrs());
        ...
        String dokterNama = tbObat.getValueAt(tbObat.getSelectedRow(), 12).toString();
        String tglSurat = tbObat.getValueAt(tbObat.getSelectedRow(), 8).toString();
        String tglRencana = tbObat.getValueAt(tbObat.getSelectedRow(), 10).toString();
        
        param.put("finger", "Dikeluarkan di " + akses.getnamars() + ", Kabupaten/Kota " + akses.getkabupatenrs() + 
                           "\nDitandatangani secara elektronik oleh " + dokterNama + 
                           "\nID " + tglSurat + 
                           "\n" + Valid.SetTgl3(tglRencana));
        
        // 3. Query dengan data yang disensor
        // Query dengan sensor - gunakan NULL untuk tanggal lahir
String query = "SELECT bridging_sep.no_rawat, bridging_sep.no_sep, " +
              "CONCAT('***-**-', SUBSTRING(bridging_sep.no_kartu, -4)) as no_kartu, " + // Sensor no_kartu
              "bridging_sep.nomr, " +
              "CONCAT(SUBSTRING(bridging_sep.nama_pasien, 1, 2), '***') as nama_pasien, " + // Sensor nama
              "NULL as tanggal_lahir, " + // NULL untuk date field
              "bridging_sep.jkel, " +
              "bridging_sep.diagawal, " +
              "'*** DATA DISENSOR UNTUK PRIVASI ***' as nmdiagnosaawal, " + // Sensor diagnosa
              "bridging_surat_kontrol_bpjs.tgl_surat, bridging_surat_kontrol_bpjs.no_surat, " +
              "bridging_surat_kontrol_bpjs.tgl_rencana, bridging_surat_kontrol_bpjs.kd_dokter_bpjs, " +
              "bridging_surat_kontrol_bpjs.nm_dokter_bpjs, bridging_surat_kontrol_bpjs.kd_poli_bpjs, " +
              ...
              "WHERE bridging_surat_kontrol_bpjs.no_surat = '" + noSurat + "'";

        
        // 4. Execute dan generate screenshot
        PreparedStatement ps = koneksi.prepareStatement(query);
        ResultSet rs = ps.executeQuery();
        
        if (!rs.next()) {
            JOptionPane.showMessageDialog(this, "❌ Data surat kontrol tidak ditemukan!");
            rs.close();
            ps.close();
            return null;
        }
        ...
        return convertJasperPrintToImage(jasperPrint, noRM, namaPasien, noSurat);
        
    } catch (Exception e) {
        e.printStackTrace();
        JOptionPane.showMessageDialog(this, "Error membuat screenshot: " + e.getMessage());
        return null;
    }
}


private String cariFileJasper() {
    // Cari file jasper yang sama dengan yang digunakan di MnSuratActionPerformed
    String[] possiblePaths = {
        "report/rptBridgingSuratKontrol2.jasper",
        "./report/rptBridgingSuratKontrol2.jasper", 
        "src/report/rptBridgingSuratKontrol2.jasper",
        "../report/rptBridgingSuratKontrol2.jasper"
    };
    ...
    for (String path : possiblePaths) {
        ...
        if (file.exists()) {
            System.out.println("✅ Jasper file found: " + file.getAbsolutePath());
...
private String convertJasperPrintToImage(net.sf.jasperreports.engine.JasperPrint jasperPrint, 
                                        String noRM, String namaPasien, String noSurat) {
    try {
        // Gunakan ukuran page yang sebenarnya dari jasper report
        int pageWidth = jasperPrint.getPageWidth();
        int pageHeight = jasperPrint.getPageHeight();
        
        // Scale factor untuk kualitas yang baik
        ...
        // Set rendering hints untuk kualitas terbaik
        ...
        // Scale graphics untuk zoom factor
        ...
        configuration.setPageIndex(0); // Halaman pertama saja
        exporter.setConfiguration(configuration);
        
        // Export - ini akan render semua elemen jasper termasuk logo BPJS, barcode TTD
        ...
        String cleanNoSurat = noSurat.replaceAll("[^a-zA-Z0-9]", "_");
        
        String fileName = noRM + "_" + cleanNama + "_" + cleanNoSurat + "_001.png";
        ...
        System.out.println("✅ Report image saved: " + outputFile.getAbsolutePath());
        ...
        e.printStackTrace();
        JOptionPane.showMessageDialog(null, "Error converting report to image: " + e.getMessage());
        return null;
    }
}
...
private String inputNomorWhatsApp(String defaultNomor, String namaPasien) {
    ...
    try {
        while (!validInput) {
            String message = "Kirim Screenshot Surat Kontrol ke WhatsApp\n" +
                            "Nama Pasien: " + namaPasien + "\n\n" +
                            "Masukkan nomor WhatsApp:\n" +
                            "(Format: 628xxxxxxxxx - dimulai 628, bukan 08)";
            
            // Dialog akan selalu di atas karena parent di-set always on top
            String input = (String) JOptionPane.showInputDialog(
                this,                           // parent yang sudah always on top
                message,                        
        ...
        // Reset always on top setelah selesai
...
private String formatNomorWA(String nomor) {
    ...
    if (cleanNomor.startsWith("08")) {
        ...
        // TIDAK BOLEH 0 - wajib convert ke 62
        cleanNomor = "62" + cleanNomor.substring(1);
    } else {
        // Nomor lainnya, tambah 628 di depan
...
private boolean validasiNomorWA(String nomor) {
    ...
    // WAJIB dimulai dengan 628
    if (!nomor.startsWith("628")) {
        return false;
    }
    
    // Panjang harus 12-15 digit (628 + 9-12 digit)
    if (nomor.length() < 12 || nomor.length() > 15) {
        return false;
    }
    
    // Semua karakter harus digit
...
private void simpanKeWADatabase(String nomor, String pesan, String fileName) {
    try {
        ...
        ps.setString(3, fullFilePath); // Path lengkap + nama file
        
        int result = ps.executeUpdate();
        if (result > 0) {
            JOptionPane.showMessageDialog(this, 
                "✅ Pesan WhatsApp berhasil ditambahkan ke antrian!\n\n" +
                "Nomor: " + nomor + "\n" +
                "File: " + fullFilePath + "\n" +
...
// Method khusus untuk auto format dari database ke format WA
private String autoFormatToWA(String nomor) {
    ...
    // Auto convert berdasarkan format
    if (cleanNomor.startsWith("08")) {
        // 081234567890 → 6281234567890
        ...
        // Nomor lainnya, tambah 62 di depan
        return "62" + cleanNomor;
    }
}

private String sensorNamaPasien(String namaPasien, int minChar, int maxChar) {
    ...
    for (int i = 0; i < kataNama.length; i++) {
        ...
        if (kata.length() > 0) {
            // Tentukan berapa karakter yang ditampilkan
            int charToShow = Math.min(Math.max(minChar, 1), Math.min(maxChar, kata.length()));
            
            // Tampilkan karakter awal
            namaSensor.append(kata.substring(0, charToShow).toUpperCase());
            
            // Tambahkan bintang untuk sisa karakter
            if (kata.length() > charToShow) {
                for (int j = charToShow; j < kata.length(); j++) {
                    namaSensor.append("*");
                }
            }
        }
